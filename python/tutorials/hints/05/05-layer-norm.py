...
def _layer_norm_fwd_fused(
    ...
    for off in range(0, N, BLOCK_SIZE):
        ...
        a = tl.load(X + cols, mask=cols < N, other=0.).to(tl.float32)  # @hint: shared_memory
        _mean += a
    ...
    for off in range(0, N, BLOCK_SIZE):
        cols = off + tl.arange(0, BLOCK_SIZE)
        x = tl.load(X + cols, mask=cols < N, other=0.).to(tl.float32)  # @hint: shared_memory
    ...
    for off in range(0, N, BLOCK_SIZE):
        cols = off + tl.arange(0, BLOCK_SIZE)
        mask = cols < N
        w = tl.load(W + cols, mask=mask)  # @hint: shared_memory
        b = tl.load(B + cols, mask=mask)  # @hint: shared_memory
        x = tl.load(X + cols, mask=mask, other=0.).to(tl.float32)  # @hint: shared_memory
        x_hat = (x - mean) * rstd
        y = x_hat * w + b
        # Write output
