...
def _attn_bwd_dkdv(dk, dv,  #
    ...
    for blk_idx in range(num_steps):
        qT = tl.load(qT_ptrs)  # @hint: shared_memory
        # Load m before computing qk to reduce pipeline stall.
        offs_m = curr_m + tl.arange(0, BLOCK_M1)
        m = tl.load(M + offs_m)  # @hint: shared_memory
        qkT = tl.dot(k, qT)
        ...
        do = tl.load(do_ptrs)  # @hint: shared_memory
        # Compute dV.
        ppT = pT
        ppT = ppT.to(tl.float16)
        dv += tl.dot(ppT, do)
        # D (= delta) is pre-divided by ds_scale.
        Di = tl.load(D + offs_m)  # @hint: shared_memory
        # Compute dP and dS.
...
def _attn_bwd_dq(dq, q, K, V,  #
    ...
    step_n = BLOCK_N2
    for blk_idx in range(num_steps):
        kT = tl.load(kT_ptrs)  # @hint: shared_memory
        vT = tl.load(vT_ptrs)  # @hint: shared_memory
        qk = tl.dot(q, kT)
        p = tl.math.exp2(qk - m)
        # Autoregressive masking.
        if MASK:
...
def _attn_bwd(Q, K, V, sm_scale,  #
    ...
    k = tl.load(K + offs_n[:, None] * stride_tok + offs_k[None, :] * stride_d)  # @hint: shared_memory
    v = tl.load(V + offs_n[:, None] * stride_tok + offs_k[None, :] * stride_d)  # @hint: shared_memory
    ...
    q = tl.load(Q + offs_m[:, None] * stride_tok + offs_k[None, :] * stride_d)  # @hint: shared_memory
    dq = tl.zeros([BLOCK_M2, HEAD_DIM], dtype=tl.float32)
    do = tl.load(DO + offs_m[:, None] * stride_tok + offs_k[None, :] * stride_d)  # @hint: shared_memory
