...
# TODO long and float64 are not supported on gcu300
#torch_types = ["int8", "uint8", "int16", "int32", "long", "float16", "float32", "float64"]
torch_types = ["int8", "uint8", "int16", "int32", "float16", "float32"]
...
def test_print(func_type: str, data_type: str, device: str):
    ...
    if func_type == "static_print":
        ...
        outs = [line for line in proc.stderr.decode("UTF-8").splitlines() if line]
    ...
    if func_type in ("print", "device_print", "device_print_uint", "device_print_uint_cast"):
	    ...
	    # TODO
            ...
            if (target_arch == 'gcu400' or target_arch == 'gcu410'):
                line = f"[0, 0, {i // 32}]  x: : {i + offset} (idx {i % 32})"
            if data_type.startswith("float"):
                original_str = line
                target = " (idx"
                insert_idx = original_str.find(target)
                assert insert_idx != -1, "cannot find \" (idx\" in the print strings"
                line = original_str[:insert_idx] + ".000000" + original_str[insert_idx:]
