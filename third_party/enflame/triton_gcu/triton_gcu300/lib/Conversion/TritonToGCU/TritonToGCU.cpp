...
struct TTMakeRangeOpLowering : SharedConversionPattern<triton::MakeRangeOp> {
  ...
  TTMakeRangeOpLowering(const TypeConverter &converter, MLIRContext *ctx,
      ...
      : SharedConversionPattern(converter, ctx,
...
struct TTLoadOpLowering : SharedConversionPattern<triton::LoadOp> {
  ...
  LogicalResult loadSingleElement(triton::LoadOp op, OpBuilder &builder,
                                  ...
                                  Value mask, Value other) const {
    auto loc = op.getLoc();

    auto elemType =
        dyn_cast<gcu::PtrType>(ptr.getType()).getElementType();
    ...
    builder.create<scf::IfOp>(
        ...
        [&](OpBuilder &builder, Location loc) {
          builder.create<memref::DmaStartOp>(
              loc, from, ValueRange{zero}, to,
              ValueRange{zero}, one,
              tag.getTag(), ValueRange{tag.getIdx()});
          builder.create<memref::DmaWaitOp>(
              loc, tag.getTag(), ValueRange{tag.getIdx()}, one);
  ...
  matchAndRewrite(triton::LoadOp op, OpAdaptor adaptor,
    ...
    if (isa<TensorType>(op.getType())) {
      ...
      auto offsets = syncAllocOp(
          rewriter, loc, std::make_pair(op.getOperation(), -1),
          userAnalysis, replaced2Origin,
    ...
    if (failed(loadSingleElement(op, rewriter, adaptor.getPtr(), output,
...
struct TTStoreOpLowering : SharedConversionPattern<triton::StoreOp> {
  ...
  void storeSingleElement(triton::StoreOp op, OpBuilder &builder,
                          Value ptr, Value values, Value offset,
                          triton::gcu::TagInfo tag, Value mask) const {
    auto elemType =
        dyn_cast<gcu::PtrType>(ptr.getType()).getElementType();
    auto loc = op.getLoc();
  ...
  matchAndRewrite(triton::StoreOp op, OpAdaptor adaptor,
    ...
    if (isa<TensorType>(op.getPtr().getType())) {
      auto numElems =
          triton::gcu::getElemsPerThread(op.getPtr().getType());
      auto numElemValues =
...
struct TTHistogramOpLowering : SharedConversionPattern<triton::HistogramOp> {
  ...
  matchAndRewrite(triton::HistogramOp op, OpAdaptor adaptor,
    ...
    if (dyn_cast<TensorType>(op.getOperand(0).getType()).getShape()[0] <
        warpsPerCTA[0])
      warpsWalkNum = dyn_cast<TensorType>(op.getOperand(0).getType())
                         .getShape()[0];
...
struct GCULoadOpLowering : SharedConversionPattern<triton::gcu::LoadOp> {
  ...
  matchAndRewrite(triton::gcu::LoadOp op, OpAdaptor adaptor,
    ...
    auto outBroadcast = syncAllocOp(rewriter, loc, lastUser,
                        userAnalysis, replaced2Origin, resultType);
    ...
    auto total_size = rewriter.create<scf::IfOp>(
        ...
        [&](OpBuilder builder, Location loc) {
          ...
          if (attr && cast<BoolAttr>(attr).getValue()) {
              load_size = ConfigGcuLoadEx(
                ...
                outBroadcast, op, resultType, adaptor.getPtr(),
                adaptor.getStrides(), adaptor.getShape(), defaultValue, tag,
                IsShareOutput);
          } else {
             load_size = ConfigGcuLoad(
              builder, loc, pTagPool, output, outBroadcast,
              op, resultType, adaptor.getPtr(), adaptor.getStrides(),
        ...
        [&](OpBuilder &builder, Location loc) {
          ...
          doMemset(builder, pTagPool, op, output, defaultValue, totalNumElems);
...
struct GCUStoreOpLowering : SharedConversionPattern<triton::gcu::StoreOp> {
  ...
  matchAndRewrite(triton::gcu::StoreOp op, OpAdaptor adaptor,
    ...
    auto storeTransType = MemRefType::get(
      storeValueType.getShape(), storeValueType.getElementType());
    auto storeTrans =
        syncAllocOp(rewriter, loc, std::make_pair(nullptr, -1),
                    userAnalysis, replaced2Origin, storeTransType);
    auto tag = isLastOp ?
                    pTagPool.getSyncTagInfo(op) :
...
struct TTExpandDimsOpLowering
  ...
  matchAndRewrite(triton::ExpandDimsOp op, OpAdaptor adaptor,
    ...
    // move source to shared memory
    auto sharedSrc =
        storeToSharedMem(rewriter, tag, srcType, adaptor.getSrc(),
                         false, std::make_pair(op.getOperation(), -1),
                         userAnalysis, replaced2Origin);
    ...
    Value output = loadFromSharedMem(
        rewriter, tag, op.getType(), result, false, lastUser,
        std::make_pair(nullptr, -1), userAnalysis, replaced2Origin);
    leaveTritionOp(rewriter, op.getOperation());
    rewriter.replaceOp(op, output);
    return success();
...
struct TTGConvertLayoutOpLowering
  ...
  matchAndRewrite(triton::gpu::ConvertLayoutOp op, OpAdaptor adaptor,
    ...
    if (mlir::isa<triton::gpu::SharedEncodingTrait>(srcLayout) &&
      ...
      // copy to local
      auto output = loadFromSharedMem(
          rewriter, tag, op.getResult().getType(), adaptor.getSrc(),
          false, lastUser, firstUser, userAnalysis, replaced2Origin);
      leaveTritionOp(rewriter, op.getOperation());
      rewriter.replaceOp(op, output);
      return success();
    } else if (isa<triton::gpu::BlockedEncodingAttr>(srcLayout) &&
      ...
      auto sharedSrc = storeToSharedMem(
          rewriter, tag,
          dyn_cast<TensorType>(op.getSrc().getType()),
          adaptor.getSrc(), false, lastUser, userAnalysis,
          replaced2Origin);
      ...
      auto sharedSrc = storeToSharedMem(
          ...
          replaced2Origin);
      // copy back outputs
      auto output = loadFromSharedMem(rewriter, tag,
...
struct TTDotOpLowering : SharedConversionPattern<triton::DotOp> {
  ...
  matchAndRewrite(triton::DotOp op, OpAdaptor adaptor,
    ...
    auto output = syncAllocOp(rewriter, loc, lastUser, userAnalysis,
                              replaced2Origin, resultMemRefType);
    if (op.getType().getRank() == 2) {
      rewriter.create<gcu::MatMulOp>(loc, output,
      ...
      scf::buildLoopNest(
          ...
          [&](OpBuilder &rewriter, Location loc, ValueRange m) {
            ...
            rewriter.create<gcu::MatMulOp>(loc, newOutMemRef,
...
void ConvertTritonToGCUPass::runOnOperation() {
  ...
  getPipelineAsyncResourceMaping(moduleOp, asyncLoad2Tag,
                                 asyncLoad2TransBuffers,
                                 asyncLoad2TagIdex, asyncWait2Tag);
  std::map<Operation *, std::map<uint64_t, bool>>
                                TTYeiledOPerandHasMultiUseStage;
  ...
  patterns.add<TTAsyncLoadFromGlobalOpLowering>(
      converter, ctx, userAnalysis, replaced2Origin, pTagPool,
      asyncLoad2Tag, asyncLoad2TagIdex, asyncLoad2TransBuffers);
