...
createSchedule(scf::ForOp forOp, int numStages) {
  ...
  for (Operation &op : forOp.getBody()->without_terminator()) {
    ...
    if (isa<scf::ForOp>(op))
      foundLoop = true;
    if (isa<scf::ForOp, mlir::triton::gcu::AsyncWaitOp>(op))
      continue;
    if (foundLoop)
      epilogue.insert(&op);
  }
...
static void hoistAllocAndConst(scf::ForOp forOp) {
  ...
  for (Operation &op : forOp.getBody()->without_terminator()) {
    if (auto allocOp = dyn_cast<mlir::triton::gcu::BufferAllocOp>(op)) {
  ...
  for (Operation *op : toHoist) {
    op->moveBefore(forOp);
    auto allocOp = dyn_cast<mlir::triton::gcu::BufferAllocOp>(op);
    if (!allocOp)
      continue;
    for (Operation *user : allocOp->getUsers()) {
      if (auto dealloc = dyn_cast<mlir::triton::gcu::BufferDeallocOp>(user)) {
...
bool mlir::triton::gcu::getOuterLoopSchedule(
  ...
  options.predicateFn = mlir::triton::gcu::predicateOp;
