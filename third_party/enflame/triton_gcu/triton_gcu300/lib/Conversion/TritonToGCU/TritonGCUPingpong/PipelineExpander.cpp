...
protected:
  ...
  Value step;
  bool dynamicLoop;
  triton::gcu::PipeliningOption::AnnotationlFnType annotateFn = nullptr;
  bool peelEpilogue;
  triton::gcu::PipeliningOption::PredicateOpFnType predicateFn = nullptr;
...
public:
  ...
  bool initializeLoopInfo(ForOp op,
                          const triton::gcu::PipeliningOption &options);
...
bool LoopPipelinerInternal::initializeLoopInfo(
    ForOp op, const triton::gcu::PipeliningOption &options) {
  LDBG("Start gcu  initializeLoopInfo");
  forOp = op;
...
void LoopPipelinerInternal::emitPrologue(RewriterBase &rewriter) {
  ...
  for (int64_t i = 0; i < maxStage; i++) {
    ...
    for (Operation *op : opOrder) {
      ...
      // only for gcu to avoid dte offset > src buffer_address
      if (i > 0 && isa<triton::gcu::AsyncLoadFromGlobalOp>(newOp)) {
        newOp->setAttr("Prologue_stage_idex", rewriter.getI32IntegerAttr(i));
      }
      ...
      if (annotateFn)
        annotateFn(newOp,
                   triton::gcu::PipeliningOption::PipelinerPart::Prologue, i);
...
LoopPipelinerInternal::analyzeCrossStageValues() {
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "analyzeCrossStageValues enter:"
                 << " \n";
  });
  for (Operation *op : opOrder) {
    ...
    auto analyzeOperand = [&](OpOperand &operand) {
      ...
      if (defStage == stages.end() || defStage->second == stage ||
        ...
        return;
      if (defStage->second == stage + distance) {
        llvm::dbgs() << "current op stage:" << stage << ",current op:";
        op->dump();
        llvm::dbgs() << "defStage stage:" << defStage->second << ",def op:";
        def->dump();
      }
      ...
      LLVM_DEBUG({
        llvm::dbgs() << "info.defStage:" << info.defStage << " \n";
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "analyzeCrossStageValues leave"
                 << " \n";
  });
...
scf::ForOp LoopPipelinerInternal::createKernelLoop(
  ...
  for (auto escape : crossStageValues) {
    ...
    for (unsigned stageIdx = 0; stageIdx < info.lastUseStage - info.defStage;
      ...
      newLoopArg.push_back(valueVersion);
      LLVM_DEBUG({
        llvm::dbgs() << "createKernelLoop add loop arg: stageIdx:" << stageIdx
                     << ",mapping from :"
                     << maxStage - info.lastUseStage + stageIdx << " \n";
        llvm::dbgs() << "createKernelLoop add loop arg value: ";
        valueVersion.dump();
      });
...
LogicalResult LoopPipelinerInternal::createKernel(
  ...
  if (!peelEpilogue) {
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "createKernel &&!peelEpilogue maxStage:" << maxStage
                   << "\n";
    });
  ...
  for (Operation *op : opOrder) {
    ...
    for (OpOperand *operand : operands) {
      ...
      LLVM_DEBUG({
        llvm::dbgs() << "operands defined in previous stage update operand "
                        "from loopArgMap:";
        newForOp.getRegionIterArgs()[remap->second].dump();
      });
      nestedNewOp->setOperand(operand->getOperandNumber(),
    ...
    rewriter.setInsertionPointAfter(newOp);
    if (annotateFn)
      annotateFn(newOp, triton::gcu::PipeliningOption::PipelinerPart::Kernel,
                 0);
  }
  ...
  for (OpOperand &yieldOperand :
    ...
    // defStage.
    LLVM_DEBUG({
      llvm::dbgs() << "get old forop operand mapping:\n";
      source.dump();
    });
    if (!peelEpilogue &&
      ...
      if (def) {
        ...
        if (defStage != stages.end() && defStage->second < maxStage) {
          ...
          LLVM_DEBUG({
            llvm::dbgs() << "select source to avoid out of range: ";
            def->dump();
          });
        }
      }
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "push finnal yeiled operand";
      source.dump();
    });
  ...
  for (auto &it : crossStageValues) {
    ...
    // add extra arg.
    LLVM_DEBUG({
      llvm::dbgs() << "createKernel crossStageValue:"
                   << "version:" << version
                   << "numVersionReturned:" << numVersionReturned
                   << ",crossvalue";
      it.first.dump();
    });
    for (unsigned i = 1; i < numVersionReturned; i++) {
      setValueMapping(it.first, newForOp->getResult(yieldOperands.size()),
                      version++);
      LLVM_DEBUG({
        llvm::dbgs() << "push cross stage arg:";
        newForOp.getBody()
            ->getArguments()[yieldOperands.size() + 1 +
    ...
    setValueMapping(it.first, newForOp->getResult(yieldOperands.size()),
                    version++);
    LLVM_DEBUG({
      llvm::dbgs() << "push old to new mapping value:";
      mapping.lookupOrDefault(it.first).dump();
    });
...
void LoopPipelinerInternal::emitEpilogue(
  ...
  for (int64_t i = 1; i <= maxStage; i++) {
    for (Operation *op : opOrder) {
      ...
      if (annotateFn)
        annotateFn(newOp,
                   triton::gcu::PipeliningOption::PipelinerPart::Epilogue,
                   i - 1);
      for (unsigned destId : llvm::seq(unsigned(0), op->getNumResults())) {
        setValueMapping(op->getResult(destId), newOp->getResult(destId),
...
FailureOr<ForOp>
mlir::triton::gcu::pipelineForLoop(RewriterBase &rewriter, ForOp forOp,
                                   const triton::gcu::PipeliningOption &options,
                                   bool *modifiedIR) {
  if (modifiedIR)
    *modifiedIR = false;
  LoopPipelinerInternal pipeliner;
  LLVM_DEBUG({ llvm::dbgs() << "gcu:pipelineForLoop\n"; });
  ...
  // 1. Emit prologue.
  LLVM_DEBUG({
    llvm::dbgs() << "before emitPrologue\n";
    forOp.getOperation()->getParentOp()->dump();
  });
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "before createKernelLoop\n";
    forOp.getOperation()->getParentOp()->dump();
  });
  ForOp newForOp =
