...
struct ConvertExternElementwiseOp
  ...
  LogicalResult matchAndRewrite(triton::ExternElementwiseOp op,
    ...
    if (symbol == "__nv_logf") {
      rewriter.replaceOp(op,
                         rewriter.create<math::LogOp>(loc, op->getOperand(0)));
      return success();
    } else if (symbol == "__nv_expf") {
      rewriter.replaceOp(op,
                         rewriter.create<math::ExpOp>(loc, op->getOperand(0)));
      return success();
    } else {
      return failure();
    }
...
struct GCUCombineOps : public impl::GCUCombineOpsBase<GCUCombineOps> {
  ...
  void runOnOperation() override {
    ...
    patterns.add<ConvertExternElementwiseOp, CombineSigmoidPattern,
                 CombineSoftplusPattern>(context);
