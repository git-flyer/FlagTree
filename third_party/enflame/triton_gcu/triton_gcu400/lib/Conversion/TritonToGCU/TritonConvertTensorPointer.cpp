...
public:
  ...
  Value getExpandedOffsetWithRange(OpBuilder &builder, const Location &loc,
    ...
    auto srcEncoding =
        cast<triton::gpu::DistributedEncodingTrait>(encoding);
    ...
    auto indexI32RowType = RankedTensorType::get({tensorShape[i]},
                                    builder.getI32Type(), srcEncoding);
...
struct ConvertTensorPointerPass
  : public mlir::impl::ConvertTensorPointerPassBase<ConvertTensorPointerPass> {
  using Base::Base;
  void runOnOperation() override;

  Operation *rewriteMakeTensorPtrOp(OpBuilder &builder,
              triton::MakeTensorPtrOp op, std::stack<Operation *> &eraser);
...
Operation* ConvertTensorPointerPass::rewriteMakeTensorPtrOp(OpBuilder &builder,
                  triton::MakeTensorPtrOp op, std::stack<Operation *> &eraser) {
    ...
    for (auto dim : op.getShape()) {
      auto i32Dim = builder.create<arith::TruncIOp>(
          op.getLoc(), builder.getI32Type(), dim);
...
Operation* ConvertTensorPointerPass::rewriteAdvanceOp(OpBuilder &builder,
                  triton::AdvanceOp op, std::stack<Operation *> &eraser) {
...
Operation* ConvertTensorPointerPass::rewriteLoadStoreOp(OpBuilder &builder,
                  Operation *op, std::stack<Operation *> &eraser) {
...
Operation* ConvertTensorPointerPass::rewriteIfOp(OpBuilder &builder,
                  scf::IfOp op, std::stack<Operation *> &eraser) {
...
Operation* ConvertTensorPointerPass::rewriteForOp(OpBuilder &builder,
                  scf::ForOp op, std::stack<Operation *> &eraser) {
...
Operation* ConvertTensorPointerPass::rewriteYieldOp(OpBuilder &builder,
                  scf::YieldOp op, std::stack<Operation *> &eraser) {
...
Operation* ConvertTensorPointerPass::rewriteOp(Operation *op,
                  std::stack<Operation *> &eraser) {
