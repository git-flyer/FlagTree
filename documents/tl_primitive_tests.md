[[中文版](./tl_primitive_tests_cn.md)|English]

## FlagTree Language Primitives and Test Suite

This document enumerates the set of `triton.language` primitives and the complete test suite that a backend integrating with FlagTree is expected to support.
Helper predicates such as `is_cuda()`, `is_hip*()`, and `torch.cuda.get_device_capability()` in the test files are used to specialize or skip tests for different backends.
Total primitives: 167, core primitives: 120, optional primitives: 47 (marked as `Optional` in this document).

---

## `python/test/unit/language/test_core.py`

**Scope**: Core language primitives – arithmetic, memory, control flow, reductions, dot products, atomics, type casting, tensor ops, scheduling hints, etc.
...
**Scope**: High-level "standard" Triton ops implemented in `triton.language.standard`.
...
**Scope**: CUDA libdevice math functions exposed through `triton.language.extra.libdevice`.
...
**Scope**: Matrix multiplication and related low‑precision / scaling variants.
...
**Scope**: Random number generation primitives.
...
**Scope**: Type conversions, especially low‑precision formats such as FP8.
...
**Scope**: Block pointer creation, advanced indexing, and boundary checking.
...
**Scope**: Tuple construction, indexing, assignments, and returns in Triton kernels.
...
**Optional tests**
**Scope**: Tensor descriptor creation and usage, including ND and rank‑reducing cases and TMA (Tensor Memory Accelerator) paths.
...
**Scope**: Pipelined kernels and dot products with additional epilogues and quantized formats.
